...
Prints the platform deployment record bundled with this kit, **by generation and by chain**:
release tag, freeze commit, factory address, launch-access state, the size of the Robinhood
stock-token quote reference, and the RC5 canary metadata proof summary. No chain is ever omitted —
a chain a generation is not deployed on prints a row that says so in words, because a missing row
reads as fine.
...
This command is offline. It does not poll a chain. What it prints is
`packages/project-schema/src/deployments.js`, which is generated by `npm run kit:deployments:sync`
and re-verified by `npm run kit:deployments:check` — the state as of this commit, not a live read.
If you need the state at this instant, read `launchAccessState()` off the factory yourself: it
returns `1` for `PUBLIC` and `0` for `PREPARED`, and it is the authority over any file.
...
The complete official Robinhood stock/ETF quote reference bundled with this kit is exported from
`packages/project-schema/src/robinhood-stock-tokens.js`; `relics status` prints its size and the
version of the snapshot it came from. For example:
