...
**Can I launch today?**
On **Ethereum (1)**, **Base (8453)** and **Robinhood Chain (4663)**, yes: RC6 is deployed on all
three and every one of their factories reads `PUBLIC`, so they accept ordinary, permissionless
creator launches. On BNB Smart Chain (56), no — RC6 is not deployed there and no date is set.
Building, validating and exporting a real `.relics` file works everywhere and is not wasted work
either way, because a bundle describes a project rather than a protocol release. Whichever chain you
are on, read `launchAccessState()` from the factory before you broadcast rather than trusting this
sentence's age.
