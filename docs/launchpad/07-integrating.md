...
Two cautions. **The registry describes the RC5 deployment**, because that is the generation that is
on chain; it is not a statement about RC6 readiness. And **`PREPARED` is not "temporarily busy"** —
it is the factory's zero value, so a factory is closed from the block it is initialized and opens
exactly once, permanently. Build your integration to handle both states honestly rather than
special-casing them away, and read the factory's own state rather than a package constant when the
answer matters.
...
succeeds — and today no RC6 factory exists to broadcast to.
