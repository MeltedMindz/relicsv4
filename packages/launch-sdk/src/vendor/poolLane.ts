...
import { getAddress, keccak256, encodeAbiParameters, type Address, type Hex } from "viem";
...
export const PROTOCOL_LANES: readonly ProtocolLane[] = ["RC5", "RC6"] as const;
 ...
 */
export const LANE_POOL_FEE: Readonly<Record<ProtocolLane, number>> = Object.freeze({
  RC5: POOL_FEE_PIPS,
 ...
 */
export const LANE_HOOK_FLAGS: Readonly<Record<ProtocolLane, number>> = Object.freeze({
  RC5: 0x1440,
  RC6: 0x14c0,
});

/** True when this lane's pools are dynamic-fee pools. */
export function laneIsDynamicFee(lane: ProtocolLane): boolean {
  return LANE_POOL_FEE[lane] === DYNAMIC_FEE_FLAG;
}

function assertLane(lane: ProtocolLane): ProtocolLane {
 ...
 */
export function poolKeyForLane(lane: ProtocolLane, token: Address, hook: Address, counterAsset: Address): PoolKey {
  assertLane(lane);
...
export function poolIdForKey(key: PoolKey): Hex {
  return keccak256(
    encodeAbiParameters(
...
export function poolIdForLane(lane: ProtocolLane, token: Address, hook: Address, counterAsset: Address): Hex {
