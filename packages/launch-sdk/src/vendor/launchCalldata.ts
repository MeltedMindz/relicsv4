...
import { launchParamsAsTuple, type LaunchParams } from "./types.js";
 ...
 */
export function launchParamsComponentsOfFunction(abi: Abi, functionName: string): AbiComponent[] | null {
  const entry = (abi as unknown[]).find(
    (e) => (e as { type?: string; name?: string }).type === "function" && (e as { name?: string }).name === functionName,
  ) as { inputs?: AbiComponent[] } | undefined;
...
export function launchParamsComponentsOf(abi: Abi): AbiComponent[] | null {
 ...
 */
export function assertAbiMatchesLaunchParams(abi: Abi, context = "this ABI"): void {
 ...
 */
export function assertAbiMatchesLaunchParamsOn(abi: Abi, functionName: string, context = "this ABI"): void {
  ...
  const abiFields: string[] = components.map((c) => c.name);
  const sdkFields: string[] = [...RC6_LAUNCH_PARAMS_FIELDS];
  if (abiFields.length === sdkFields.length && abiFields.every((name, index) => name === sdkFields[index])) return;
  ...
  throw new LaunchAbiGenerationMismatchError(
    `${context} describes a different LaunchParams than this SDK encodes. ${detail} ` +
      "If you meant the DEPLOYED RC5 factory, its launch() takes a different struct and RC6 params cannot be sent to it; " +
      "if you meant RC6, load sdk/contracts-abi/rc6/LaunchpadFactoryV1.json (Rc6LaunchpadFactoryArtifact()).",
    abiFields,
    sdkFields,
  );
}
 ...
 */
export function encodeLaunchCalldata(abi: Abi, params: LaunchParams, context = "the supplied ABI"): { data: Hex; args: readonly unknown[] } {
  assertAbiMatchesLaunchParams(abi, context);
  const args = [launchParamsAsTuple(params)] as const;
