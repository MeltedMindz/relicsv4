...
// contract.
import { concatHex, encodeAbiParameters, encodePacked, getAddress, getCreate2Address, keccak256, numberToHex, size, } from "viem";
import { WETH } from "./constants.js";
import { launchHookSalt, launcherSalt } from "./hookMiner.js";
import { poolIdForKey, poolKeyForLane } from "./poolLane.js";
/**
 ...
 */
export function libCloneInitCode(implementation, args) {
    const n = size(args);
    if (n >= 0xffd3)
        throw new Error(`libCloneInitCode: args too large (${n} bytes)`);
...
export function libCloneInitCodeHash(implementation, args) {
...
export function encodeTokenArgs(name, symbol, totalSupply, mintRecipient) {
    return encodeAbiParameters([{ type: "string" }, { type: "string" }, { type: "uint256" }, { type: "address" }], [name, symbol, totalSupply, mintRecipient]);
}
 ...
 */
export function encodeHookConstructorArgs(factory, poolManager, locker, conversionSender) {
    return encodeAbiParameters([{ type: "address" }, { type: "address" }, { type: "address" }, { type: "address" }], [factory, poolManager, locker, conversionSender ?? locker]);
}
/**
 ...
 */
export function encodeRc6HookConstructorArgs(factory, poolManager, authorizedLp, conversionSender) {
    if (!conversionSender || conversionSender === "0x0000000000000000000000000000000000000000") {
        throw new Error("encodeRc6HookConstructorArgs: `conversionSender` is required. RC6's ArtHook excludes this " +
    ...
    if (conversionSender.toLowerCase() === authorizedLp.toLowerCase()) {
        throw new Error("encodeRc6HookConstructorArgs: `conversionSender` must not equal `authorizedLp`. On RC6 the " +
            "two roles are held by different contracts on BOTH lanes; collapsing them is finding J-1.");
    }
    return encodeAbiParameters([{ type: "address" }, { type: "address" }, { type: "address" }, { type: "address" }], [factory, poolManager, authorizedLp, conversionSender]);
}
/**
...
export function predictHookAddress(opts) {
    const constructorArgs = encodeHookConstructorArgs(opts.factory, opts.poolManager, opts.locker, opts.conversionSender);
 ...
 */
export function collectionSalt(token, hook) {
 ...
 */
export function helperCollectionSalt(factory, token, hook) {
    return keccak256(encodeAbiParameters([{ type: "address" }, { type: "bytes32" }], [getAddress(factory), collectionSalt(token, hook)]));
}
/**
...
export function cloneInitCode(implementation) {
    return concatHex([
        ...
        getAddress(implementation).toLowerCase(),
 ...
 */
export function computeCollectionInitCodeHash(collectionImplementation) {
 ...
 */
export function poolKeyFor(lane, token, hook, counterAsset = WETH) {
