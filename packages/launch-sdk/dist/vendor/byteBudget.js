...
function linearModelGas(scriptBytes) {
...
export function estimateGasForSize(scriptBytes) {
    const exact = MEASURED_POINTS.find((p) => p.scriptBytes === scriptBytes);
    if (exact)
        return { gas: exact.totalTx, source: "measured-exact" };
 ...
 */
export function meterScriptBytes(scriptBytes, gasPriceGwei = 20) {
