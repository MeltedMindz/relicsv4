...
import { concatHex, encodeAbiParameters, getCreate2Address, keccak256, numberToHex, } from "viem";
 ...
 */
export function namespacedHookSalt(caller, salt) {
 ...
 */
export function launcherSalt(launcher, salt) {
 ...
 */
export function launchHookSalt(caller, launcher, salt) {
...
export function mineHookSalt(opts) {
    ...
    if (typeof opts.flags !== "bigint") {
        throw new Error("mineHookSalt: `flags` is required and must be a bigint. Pass EXPECTED_HOOK_FLAGS for an " +
    ...
    if (!opts.launcher) {
        throw new Error("mineHookSalt: `launcher` is required (M-01). Every salt the factory consumes is " +
 ...
 */
export async function mineHookSaltAvoidingCollision(opts) {
