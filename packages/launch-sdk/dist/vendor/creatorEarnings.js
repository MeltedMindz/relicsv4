...
export const NO_CREATOR_EARNINGS = { mode: CreatorEarningsMode.NONE, royaltyBps: 0 };
...
export function creatorEarningsProblems(election) {
    const problems = [];
    ...
    if (mode === CreatorEarningsMode.NONE) {
        if (royaltyBps !== 0)
            problems.push("creatorEarnings: NONE must carry royaltyBps 0");
        if (policyVersion !== 0)
            problems.push("creatorEarnings: NONE must carry no policy version");
        return problems;
    }
...
export function packCreatorEarnings(election) {
    const problems = creatorEarningsProblems(election);
    if (problems.length > 0)
        throw new CreatorEarningsElectionError(problems.join("; "));
    return (BigInt(election.mode) |
...
export function unpackCreatorEarnings(packed) {
