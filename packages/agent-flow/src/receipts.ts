...
export function receiptsPath(workspace: string): string {
 ...
 */
export function listReceipts(workspace: string): StoredReceipt[] {
  const dir = receiptsPath(workspace);
  if (!existsSync(dir)) return [];
  return readdirSync(dir)
    .filter((f) => f.endsWith(".json"))
    .sort((a, b) => Number(a.split("-")[0]) - Number(b.split("-")[0]))
    .map((f) => JSON.parse(readFileSync(join(dir, f), "utf8")) as StoredReceipt);
}

export function hashOfReceipt(r: StoredReceipt): string {
  return sha256(canonical(r));
}

/** Append one receipt, linked to the current tip. */
export function writeReceipt(workspace: string, input: ReceiptBody & { inputHash?: string }): StoredReceipt {
  const dir = receiptsPath(workspace);
 ...
 */
export function verifyReceiptChain(workspace: string): ChainIntegrity {
  const receipts = listReceipts(workspace);
  if (receipts.length === 0) return { intact: true, length: 0, brokenAt: null, detail: "no receipts yet" };
...
export function latestReceipt(workspace: string, phase: string): StoredReceipt | null {
