...
function intentPath(workspace) {
    return join(workspace, AGENT_DIR, "broadcast-intent.json");
}
export function readIntent(workspace) {
...
export function writeIntent(workspace, intent) {
    mkdirSync(join(workspace, AGENT_DIR), { recursive: true });
    const full = { version: 1, writtenAt: new Date().toISOString(), ...intent };
    writeFileSync(intentPath(workspace), `${JSON.stringify(full, null, 2)}\n`);
    return full;
}
export function recordIntentTxHash(workspace, txHash) {
    const intent = readIntent(workspace);
    if (!intent)
        throw new Error("recordIntentTxHash: no broadcast intent on disk");
 ...
 */
export async function decideResend(client, intent, opts) {
    const evidence = [];
    // ---- 1. the recorded hash ---------------------------------------------------------------------
    let txHash = intent.txHash ?? null;
    if (txHash) {
        try {
    ...
    if (intent.totalLaunchesAtIntent !== null) {
        try {
            const now = (await client.readContract({ address: intent.factory, abi: opts.factoryAbi, functionName: "totalLaunches" }));
