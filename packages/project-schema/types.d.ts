...
export interface PlatformGeneration {
  ...
  /** The generation that replaced this one, or `null` when this IS the current generation. */
  supersededBy: PlatformGenerationId | null;
...
/** The LIVE generation record. Same object as `PLATFORM_GENERATIONS[CURRENT_DEPLOYED_GENERATION]`. */
export const PLATFORM_RELEASE: Readonly<PlatformGeneration>;
/** WITHDRAWN — every chain `null`. RC5 is superseded; its addresses are not published here. */
export const RC5_DEPLOYMENTS: Readonly<Record<SupportedChainId, Readonly<PlatformDeployment> | null>>;
/** GENERATED by scripts/sync-deployments.mjs from the launchpad's chain profiles. Null for a chain
 *  whose profile does not state that its addresses were read back off the chain. */
